...
在局域网内通过 WiFi 将照片批量传输到手机相册，无需数据线。

**使用方式**

1. 在桌面端选择照片所在文件夹，点击「开始传输」
2. 手机扫描二维码或在浏览器输入显示的地址（**必须带 `https://` 前缀**）
3. 手机浏览器会显示证书安全警告（因使用自签名证书），点击「高级」→「继续访问」信任即可（每台设备仅需一次）
4. 在手机页面勾选想要保存的照片，点击「保存 N 张到相册」
5. 系统分享面板弹出后，选择「存储 N 张图像」，照片直接存入手机相册

**iPhone 使用说明**

- 使用 **Safari** 打开（Web Share API 需要 Safari，Chrome for iOS 不支持）
- 首次访问地址时 Safari 会提示"此连接不安全"，点击「高级」→「继续前往...（不安全）」，之后每次访问不再提示
- 保存时选择 iOS 系统分享面板中的「存储 N 张图像」，图片直接保存到「照片」App

> **注意**：桌面端本地访问调试时，浏览器地址栏须输入 `https://127.0.0.1:8765`（带 `https://` 前缀），直接输入 IP 不加协议前缀默认走 HTTP，会导致连接失败。
