...
对文件夹内的图片批量执行模块化处理流水线，可按需开启以下模块，输出到独立子文件夹，**不影响原始文件**。

**重命名模块**（可选）

| 命名模式 | 效果示例 |
|---------|---------|
| 前缀 + 序号 | `photo_001.jpg`、`photo_002.jpg` … |
| 保持原名 + 后缀 | `DSC_0001_edit.jpg` |
| 前缀 + 序号 + 后缀 | `trip_001_edit.jpg` |

可自定义前缀/后缀、起始编号、补零位数，文件列表预览实时更新。

**压缩模块**（默认开启）

- JPEG / WEBP / BMP / TIFF → 按指定质量（1–100）重新编码为 JPEG
- PNG → 保持 PNG 格式无损输出，或转换为 JPEG（可单独设置质量）
- HEIF / HIF / AVIF → 不支持压缩，自动回退为仅重命名/直接复制

**支持的输入格式**

`.jpg` `.jpeg` `.png` `.webp` `.bmp` `.tif` `.tiff` `.hif` `.heic` `.heif` `.avif`

**输出**：在源文件夹下自动创建子文件夹（默认 `processed/`），所有处理结果写入其中。
